...
import com.example.bat.databinding.ActivityBuatPesananTelurBinding
...
class BuatPesananTelurActivity : AppCompatActivity() {
    lateinit var binding : ActivityBuatPesananTelurBinding
    var hargaAyam = 0
    var totalHarga = 0
    var diskon5 = 0
    var diskon10 = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityBuatPesananTelurBinding.inflate(layoutInflater)
        ...
        ambilHargaTelur(){
            when (it){
                true -> {
                    binding.tvHarga.text = "Harga per butir : Rp " + NumberFormat.getNumberInstance(Locale.getDefault()).format(hargaAyam)
                }
                false ->{}
            }
        }

        binding.itButir.setOnEditorActionListener { _, actionId, _ ->
            binding.itButir.clearFocus()
            if (actionId == EditorInfo.IME_ACTION_DONE) {
                if (binding.itButir.text.toString().isNullOrBlank()){
                    binding.tvTotalHarga.text = "Total harga : Rp 0"
                } else {
                    totalHarga = hargaAyam * binding.itButir.text.toString().toInt()
        ...
        binding.itButir.addTextChangedListener(object : TextWatcher {
...
//        END SELECTOR BERAT AYAM
        ...
        binding.btnBeli.setOnClickListener {
            if (binding.itNama.text.isNullOrBlank() || binding.itButir.text.isNullOrBlank()){
                ...
                buatPesanan(
                    binding.itButir.text.toString(),
                    binding.itNama.text.toString(),
                    idAlamat, selectedDiskon.toString())
            }
        }
    ...
    private fun ambilHargaTelur(callback: (Boolean) -> Unit) {
        binding.loadingProgressBar.visibility = View.VISIBLE
        val url = "https://kumowal.my.id/api/harga_get.php?item=telur"
    ...
    private fun buatPesanan(butir: String, namaPj: String, idAlamat: String, hargaTawaran : String) {
        ...
        postData.put("jenis_pesanan","telur")
        postData.put("butir",butir)
